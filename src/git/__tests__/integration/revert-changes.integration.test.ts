...
describe('GitService integration — revertCommitChanges', () => {
  ...
  it('reverts a whole hunk containing multiple change regions (reverts all of them)', async () => {
    ...
    await svc.revertCommitChanges(hash, 'h.txt', { hunkIndex: 0 });

    // Reverting the hunk restores BOTH regions, not just one.
...
describe('GitService integration — revertCommitChanges with no-trailing-newline files', () => {
  let repo: TempRepo;
  let svc: GitService;
  ...
  it('Scenario A: reverts only the last added line, restoring "a\\nP" with no trailing newline', async () => {
    // old `a` (no EOF nl) → new `a\nP\nQ` (no EOF nl). Revert only `Q`.
    ...
    await svc.revertCommitChanges(hash, 'f', { hunkIndex: 0, lineIndices: [idxQ] });

    // Must be exactly "a\nP" — NOT "a\nP\n" (the old bug gained a trailing newline).
    expect(readBytes('f')).toBe('a\nP');
  });

  it('Scenario B: reverts only the deleted line, restoring it on its own line without fusing', async () => {
    // old `X\nold` (no EOF nl) → new `X\nnew1\nnew2` (no EOF nl). Revert only `old`.
    ...
    await svc.revertCommitChanges(hash, 'f', { hunkIndex: 0, lineIndices: [idxOld] });

    // Must be "X\nold\nnew1\nnew2" — NOT "X\noldnew1\nnew2" (old bug fused old+new1).
    expect(readBytes('f')).toBe('X\nold\nnew1\nnew2');
  });

  it('reverts a whole no-newline hunk back to the pre-commit bytes', async () => {
    const hash = commitPair('f', 'a\ntail', 'A\ntail');
    await svc.revertCommitChanges(hash, 'f');
    // Trailing newline absence is preserved through the round-trip.
    expect(readBytes('f')).toBe('a\ntail');
  });

  it('regression: a normal newline-terminated single-line revert still works', async () => {
    ...
    await svc.revertCommitChanges(hash, 'f', { hunkIndex: 0, lineIndices: [idxC] });
